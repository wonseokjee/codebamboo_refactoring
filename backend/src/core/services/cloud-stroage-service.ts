...
export class CloudStorageService {
  ...
  constructor() {
    this.storage = new Storage({
      keyFilename:
        './src/core/services/strategic-cacao-386913-af7bcc9d39b2.json',
    });
  ...
  private setFilename(nickname): string {
    ...
    return `profileImg-${nickname}-${Date.now()}`
      .replace(/^\.+/g, '')
      .replace(/^\/+/g, '')
      .replace(/\r|\n/g, '_');
  }
  ...
  async uploadFile(nickname, uploadedFile, destination: string): Promise<any> {
    const fileName =
      this.setDestination(destination) + this.setFilename(nickname);
